module HashDiff
  ...
  def self.diff(obj1, obj2, options = {}, &block)
    ...
    if obj1.is_a?(Array)
      ...
      deleted_keys.sort{|a,b| a.to_s <=> b.to_s }.each do |k|
      ...
      common_keys.sort{|a,b| a.to_s <=> b.to_s }.each {|k| result.concat(diff(obj1[k], obj2[k], opts.merge(:prefix => "#{prefix}#{k}"))) }

      # added properties
      added_keys.sort{|a,b| a.to_s <=> b.to_s }.each do |k|
        unless obj1.key?(k)
